...
export class ResetPasswordComponent {
  ...
  change() {
    if (this.ChangePasswordForm.valid) {
      ...
      this.ParamForms.nGrievanceUserId=this.uid;
      this.loginService.ResetPassword(this.ParamForms).subscribe(
        (data: any) => {
         
          if (data.status === 200) {
            
            this.snackBar.open("Password reset successfully", this.action ? this.actionButtonLabel : undefined, config);
            this.onNoClick();
